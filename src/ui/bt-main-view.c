					   ...
					   void *event_info)
{
	...
	if (ugd->bt_launch_mode == BT_LAUNCH_SEND_FILE) {
		...
		if (__bt_main_request_to_send(ugd,
						...
						void *user_data)
{
	...
	profile_h = _bt_get_registered_net_profile(ugd->connection,
						dev->bd_addr);

	if (profile_h == NULL) {
		profile_h = _bt_get_connected_net_profile(ugd->connection,
							dev->bd_addr);
		if (profile_h)
	...
	if (profile_h)
		_bt_set_profile_state_changed_cb(profile_h, dev);
	...
	if (_bt_main_is_matched_profile(ugd->search_type,
		...
		BT_DBG("Device class and search type do not match \n");
		free(dev);
	}
